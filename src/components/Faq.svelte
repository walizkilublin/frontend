<script>
  ...
  // Aktywny indeks (null = wszystkie zamknięte)
  let activeIndex = 0; // Domyślnie otwarte pierwsze pytanie
  ...
  function toggle(index) {
    // Jeśli klikasz w otwarte, zamyka się. Jeśli w inne, otwiera się nowe.
    activeIndex = activeIndex === index ? null : index;
  }
</script>

<section id="faq" class="relative w-full bg-ghost-white py-24 z-20">
    ...
    <div class="w-full lg:w-2/3 flex flex-col border-t border-border-tech">
      {#each faqs as faq, index}
        <div class="border-b border-border-tech bg-white">
          <button 
            type="button"
            class="w-full text-left py-6 px-4 md:px-8 flex justify-between items-center hover:bg-surface/50 transition-colors focus:outline-none group"
          ...
          >
            ...
            <div class="relative w-6 h-6 shrink-0 flex items-center justify-center">
              <div class="absolute w-full h-[2px] bg-vantablack transition-transform duration-300 {activeIndex === index ? 'bg-signal-orange rotate-180' : ''}"></div>
              <div class="absolute h-full w-[2px] bg-vantablack transition-transform duration-300 {activeIndex === index ? 'bg-signal-orange rotate-90 scale-0' : 'rotate-0 scale-100'}"></div>
            </div>
          </button>
